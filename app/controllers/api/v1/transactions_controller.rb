        ...
        class TransactionsController < ApplicationController
            ...
            def bulk
                ...
                if transactions.all?(&:persisted?)
                    ...
                    render json: { status: 'error', errors: transactions.map(&:errors).map(&:full_messages) }, status: :unprocessable_entity
                end
            end
            ...
            def bulk_transaction_params
                arams.permit(transactions: [:transaction_id, :points, :user_id])
            end
